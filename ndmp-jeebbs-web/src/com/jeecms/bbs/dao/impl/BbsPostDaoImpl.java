...
//		}
	...
	public BbsPost getLastPost(Integer forumId, Integer topicId) {
		String hql = "select bean from BbsPost bean where bean.topic.id!=:topicId and bean.topic.forum.id=:forumId order by bean.createTime desc";
		Finder f=Finder.create(hql).setParam("topicId", topicId).setParam("forumId", forumId);
		List<BbsPost>posts=find(f);
